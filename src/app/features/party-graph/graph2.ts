...
interface ApiAgreement {
  id: number; code: string; status: string; startDate: string; endDate: string | null;
}
...
interface GNode {
  id: string;
  kind: NodeKind;
  partyId?: number;         // set for party nodes
  accountCode?: string;     // set for account nodes
  label: string;
  sublabel: string;         // role string (party) or type+currency (account)
  tier: Tier;
  lei?: string;
  countryCode?: string;
  currency?: string;
...
// ── Layout ────────────────────────────────────────────────────────────────────
// Tiers: 0 = market/CCP, 1 = firm, 2 = client (parties), 3 = accounts
...
@Component({
                ...
                @if (node.kind === 'party') {
                  ...
                  >
                    ...
                    <div class="fn-body">
                      ...
                      <div class="fn-meta">{{ node.countryCode }} · {{ (node.lei ?? '') | slice:0:10 }}…</div>
                    </div>
  ...
  styles: [`
    ...
    .state { flex:1; display:flex; align-items:center; justify-content:center; gap:12px; color:var(--text-muted); font-size:13px; }
    .state.error { color:#f87171; }
    .state.dim   { color:var(--text-dim); }
    .spin { width:18px; height:18px; border:2px solid var(--green-faint); border-top-color:var(--green); border-radius:50%; animation:spin 0.7s linear infinite; }
    ...
    .fnode:hover { border-color:rgba(0,155,119,0.35); box-shadow:0 0 0 2px rgba(0,155,119,0.08); }
    .fnode.focal { border-color:var(--green) !important; box-shadow:0 0 0 2px rgba(0,155,119,0.2); }
...
export class PartyGraphComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    forkJoin({
      ...
      next:  ({ rels, accounts }) =>
               this.buildGraph(Number(id), rels.data, accounts.data.content),
      error: e => { this.err.set(e?.message ?? 'Failed to load'); this.loading.set(false); }
    });
  }
  ...
  private buildGraph(
    ...
    const roleMap  = new Map<number, Set<string>>();  // partyId → role names

    // ── 1. Collect party nodes from relationships ──────────────────────────
    for (const r of rels) {
      for (const side of [r.fromPartyRole, r.toPartyRole]) {
        const p = side.party;
        const nid = `p-${p.id}`;
        if (!nodeMap.has(nid)) {
          nodeMap.set(nid, {
            id: nid, kind: 'party', partyId: p.id,
            label: p.name, sublabel: '',
            tier: roleToTier(side.roleType.code),
            lei: p.lei, countryCode: p.countryCode,
            x: 0, y: 0
          });
        }
        if (!roleMap.has(p.id)) roleMap.set(p.id, new Set());
        roleMap.get(p.id)!.add(side.roleType.name);
      }
    }

    // Attach accumulated role strings
    nodeMap.forEach((n) => {
      if (n.kind === 'party' && n.partyId != null) {
        n.sublabel = [...(roleMap.get(n.partyId) ?? [])].join(', ');
      }
    });

    // ── 2. Collect relationship edges ──────────────────────────────────────
    ...
    // ── 3. Build account nodes + edges ────────────────────────────────────
    // partyRolesFull tells us which party role the account belongs to.
    // We map partyRoleId → party node via the relationship data.
    //
    // Build a lookup: partyRoleId → partyId  (from both sides of every rel)
    const roleIdToPartyId = new Map<number, number>();
    for (const r of rels) {
      roleIdToPartyId.set(r.fromPartyRole.id, r.fromPartyRole.party.id);
      roleIdToPartyId.set(r.toPartyRole.id,   r.toPartyRole.party.id);
    }
    ...
    for (const acc of accounts) {
      ...
      nodeMap.set(anid, {
        ...
        tier: 'client',           // accounts always hang below client/firm
      ...
      // Connect account to its party node via the partyRole assignment.
      // One account can be assigned to multiple partyRoles → multiple edges.
      for (const assignment of acc.assignmentsFull) {
        const ownerPartyId = roleIdToPartyId.get(assignment.partyRoleId);
        // Fallback: use focalPartyId if we can't resolve the role
        const fromNodeId = ownerPartyId
          ? `p-${ownerPartyId}`
          : `p-${focalPartyId}`;

        // Find which role this assignment belongs to, for the edge label
        const matchedRole = acc.partyRolesFull.find(
          pr => pr.id === assignment.partyRoleId
        );
        const roleLabel = matchedRole?.roleTypeName ?? 'Account';
    ...
    const focal = nodeMap.get(`p-${focalPartyId}`);
    this.focalName.set(focal?.label ?? '');
    this.focalNodeId.set(`p-${focalPartyId}`);
